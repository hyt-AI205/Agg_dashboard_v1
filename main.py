...
Responsibilities:
  ...
  • Register routers:
      /api/dashboard/*       → dashboard.router          (stats, health, config)
      /api/targets/*         → dashboard.targets_router  (CRUD for scrape targets)
  • Serve HTML pages (GET /dashboard)

All API logic lives in dashboard.py; this file only wires things together.
"""
...
# ---------------------------------------------------------------------------
try:
    from dashboard import router as dashboard_router, targets_router, init_targets_store

    # Give the targets router a reference to the live store
    ...
    raise  # Hard-fail on startup; don't silently limp along
